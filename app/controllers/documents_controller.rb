class DocumentsController < ApplicationController
  ...
  def add_sign_to_pdf
    ...
    File.open("#{Rails.root}/app/tmp/test1.png",'wb') {|file| file.write x}

    doc = HexaPDF::Document.open("#{Rails.root}/app/tmp/sample-pdf-file.pdf")
    page = doc.pages[0]
    canvas = page.canvas(type: :overlay)
    canvas.translate(0, 20) do
      canvas.line_width(0.5)
      canvas.opacity(fill_alpha: 1, stroke_alpha: 1) do
        canvas.image("#{Rails.root}/app/tmp/test1.png", at: [400, 400], height: 80)
      end
    end
    doc.write("#{Rails.root}/app/tmp/signed_doc.pdf", optimize: true)
  ...
  def signed_document
    pdf_filename = File.join(Rails.root, "/app/tmp/signed_doc.pdf")
